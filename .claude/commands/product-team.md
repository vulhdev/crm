You are orchestrating the **product-team** for the CRM platform. The team has three specialized agents:

- **product-owner** — defines the PRD: problem statement, user stories, and acceptance criteria **before** any design or QA work begins
- **product-designer** — maps user flows, wireframes, and interaction design **after** the PRD is ready
- **qa-analyst** — produces acceptance test scenarios, risk assessment, and quality exit criteria **after** the PRD and UX Brief are ready
...
**All features follow this order: Product Owner → Product Designer → QA Analyst.**

---

## Output folder

Before spawning any agent, derive a kebab-case slug from the feature name (e.g. `customer-filters`) and create a dedicated output folder:
...
4. Spawn `product-designer` — pass the PRD + relevant doc content + output path. Returns a **UX Brief** saved to `ux-brief.md`.
5. Spawn `qa-analyst` — pass the PRD + UX Brief + relevant doc content + output path. Returns a **Test Plan** saved to `test-plan.md`.

### Parallel design + QA (requirements already defined externally):
1. Read `.claude/docs/` (see above).
2. Derive the feature slug and create `.claude/docs/<feature-slug>/`.
3. Spawn `product-designer` + `qa-analyst` **in parallel** — both receive the existing requirements doc + relevant doc content + their respective output paths.
...
4. Spawn `product-designer` — returns UX flows to expose UX unknowns, saved to `ux-brief.md`.
...
Once all three agents have finished and you have the PRD + UX Brief + Test Plan, **ask the user**:
